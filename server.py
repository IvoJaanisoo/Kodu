...
def _bisect_rent(target, irr_type, inp, lo=5.0, hi=50.0):
    ...
    # Scan upward from lo to find valid lower bound
    # (needed when low rents produce multi-sign CFs with EIS low equity)
    step = 0.5
    lo_v = get(lo)
...
def _compute(p):
    ...
    D = {
        ...
        "input_exit_override": 0.0,  # 0 = use computed D38; >0 = use this €/rentable m²
    }
    D.update(p)
    ...
    D11 = 2086  # kinnistu pind m² (fixed, cell D11)

    # Area ratios — E27 is now a user input (green cell)
    E27 = D["input_apartmentarea_share"]
    E28 = 1.0 - E27               # common = remainder (no parking in base model)
    E29 = 0.0                     # parking share = 0

    D15 = D["input_apartments"] * D["input_apartment_area"]   # D15 = D13*D14
    D16 = D15 / E27                                            # D16 = D15/E27
    D20 = D["input_site_price"] * D11 / D16                   # D20 = D19*D11/D16
    ...
    D36 = D34 + D35
    D37 = D36 * D["input_VAT"]
    D38 = D36 + D37   # construction cost per rentable m² incl VAT = loan basis

    # Exit value: use override if provided, else D38 (construction cost = conservative)
    exit_val = D["input_exit_override"] if D["input_exit_override"] > 0 else D38
    ...
    for y in range(1, N):
        ...
        r11[y] = r9[y] + r10[y]
        r12[y] = exit_val if y == D8 else 0.0   # exit uses override if set
    ...
    # J17 = loan basis = ALWAYS construction cost D38 (not exit_val)
    ...
    # Year 5 = first fully stabilised operational year
    noi_y5 = r11[5] if D8 >= 5 else (r11[D8] if D8 >= 1 else 0.0)
    if nper > 0 and abs(D6) > 1e-10:
        annual_pmt = J17 * D6 / (1 - (1 + D6) ** -nper)
    elif nper > 0:
        annual_pmt = J17 / nper
    else:
        annual_pmt = 0.0
    DSCR = round(noi_y5 / annual_pmt, 3) if annual_pmt > 0 else None
