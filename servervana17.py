...
def _irr(cfs, lo=-0.95, hi=2.0, steps=400):
    """Robust IRR via bounded scan + bisection. Returns None if no real IRR in [lo,hi].
    Eliminates spurious roots (e.g. 900%+) caused by Newton's method on ill-conditioned CFs."""
    def npv(r):
    ...
    for i in range(1, steps+1):
        ...
        if fa*fb < 0:
            ...
            return (a+b)/2
        a=b; fa=fb
    return None  # no sign change in realistic range → no real IRR
...
def _compute(p):
    ...
    posEq=sum(v for v in r20[1:D10+1] if v>0)
    negEq=sum(v for v in r20[1:D10+1] if v<0)
    eqMult=round(posEq/abs(negEq),3) if negEq<-1e-9 else None
    return {
        "eqMult":eqMult,
