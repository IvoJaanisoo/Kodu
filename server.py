...
"""
import json, os, sqlite3, re, time
...
# ── Database ─────────────────────────────────────────────────────────────────
# Set SPINNAKER_DB env var to a persistent path in production
# (Render.com: mount a Persistent Disk at /data, set SPINNAKER_DB=/data/spinnaker.db)
DB_PATH = os.environ.get("SPINNAKER_DB", "/tmp/spinnaker.db")
    ...
    with _db() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS registrations (
                ...
                type      TEXT    NOT NULL CHECK(type IN ('dev','kov','emp')),
                region    TEXT    NOT NULL,
                org       TEXT    NOT NULL,
                contact   TEXT    NOT NULL,
                email     TEXT    NOT NULL,
                loan      REAL    DEFAULT 0,
                apts      INTEGER DEFAULT 0,
...
def _save_registration(d):
    with _db() as conn:
        cur = conn.execute("""
            INSERT INTO registrations
              (type,region,org,contact,email,loan,apts,units,land,addr,anchor,arch,note,ts)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (d["type"], d["region"], d["org"], d["contact"], d["email"],
              float(d.get("loan") or 0), int(d.get("apts") or 0),
              int(d.get("units") or 0), float(d.get("land") or 0),
              str(d.get("addr") or ""), int(bool(d.get("anchor"))),
              int(bool(d.get("arch"))), str(d.get("note") or "")[:500],
              int(d.get("ts") or time.time()*1000)))
        conn.commit()
...
def _get_registrations():
    with _db() as conn:
        rows = conn.execute(
            "SELECT * FROM registrations ORDER BY ts DESC"
        ).fetchall()
    return [dict(r) for r in rows]

# ── Rate limiting (in-memory, per IP) ────────────────────────────────────────
_rl: dict = {}
def _rate_ok(ip, max_per_hour=10):
    now = time.time()
    _rl[ip] = [t for t in _rl.get(ip, []) if now - t < 3600]
    if len(_rl[ip]) >= max_per_hour:
        return False
    _rl[ip].append(now)
    return True
...
def _validate(d):
    errs = []
    if d.get("type") not in ("dev","kov","emp"):
        errs.append("type must be dev, kov, or emp")
    for f,label,maxlen in [("region","Piirkond",100),("org","Organisatsioon",200),
                            ("contact","Kontaktisik",100),("email","E-post",254)]:
        v = str(d.get(f,"")).strip()
        if not v: errs.append(f"{label} on kohustuslik")
        elif len(v)>maxlen: errs.append(f"{label} on liiga pikk (max {maxlen})")
    if d.get("email") and not _EMAIL_RE.match(str(d.get("email","")).strip()):
        errs.append("E-posti aadress on vigane")
    loan = d.get("loan", 0)
    try:
        if float(loan or 0) < 0: errs.append("Laenuvajadus peab olema positiivne arv")
    except (TypeError, ValueError):
        errs.append("Laenuvajadus peab olema arv")
    return errs
...
def _irr(cfs, lo=-0.95, hi=2.0, steps=400):
    def npv(r):
        return sum(v/(1+r)**t for t,v in enumerate(cfs))
    fa = npv(lo)
    if abs(fa)<1e-9: return lo
    a = lo
    for i in range(1, steps+1):
        b = lo+(hi-lo)*i/steps
        fb = npv(b)
        if abs(fb)<1e-9: return b
        if fa*fb < 0:
            for _ in range(80):
                m=(a+b)/2; fm=npv(m)
                if abs(fm)<1e-12 or (b-a)<1e-12: return m
...
def _compute(p):
    ...
    D10 = D8
    D11 = D["C11"]; H26=D["H26"]
    D57 = min(D["D19"], D["C57"]) if c14 else D["C57"]
...
FIELD_CASTS={
    ...
    "D22":float,"D23":int,
    "D26":int,"D28":int,"D29":float,  # D27 removed — non-configurable VAT
...
class H(BaseHTTPRequestHandler):
    ...
    def _json(self, code, obj):
        ...
        self._cors(); self._sec(); self.end_headers(); self.wfile.write(b)

    def _html(self):
        b=FRONTEND.read_bytes(); self.send_response(200)
        self.send_header("Content-Type","text/html; charset=utf-8")
        self.send_header("Content-Length",len(b))
        self._sec(); self.end_headers(); self.wfile.write(b)

    def do_OPTIONS(self):
        self.send_response(204); self._cors(); self._sec(); self.end_headers()
    ...
    def do_GET(self):
        ...
        if pt in ("/","/index.html"):
            ...
            self._json(200,{"status":"ok","engine":"v7"})
        elif pt=="/defaults":
            self._json(200,{"outputs":_compute({})})
        elif pt=="/registrations":
            try:
                entries=_get_registrations()
                # Strip email/contact from public response to protect PII
                # (only _key, type, region, org, loan, apts, units, land, addr,
                #  anchor, arch, note, ts are returned publicly)
                public=[{k:e[k] for k in
                    ("id","type","region","org","loan","apts","units",
                     "land","addr","anchor","arch","note","ts")
                    if k in e} for e in entries]
                self._json(200,{"entries":public})
    ...
    def do_POST(self):
        ...
        if pt=="/calculate":
            ...
            if not _rate_ok(ip):
                self._json(429,{"error":"Liiga palju päringuid. Proovige hiljem uuesti."}); return
            errs=_validate(data)
            if errs:
                self._json(422,{"error":"; ".join(errs)}); return
            try:
                row_id=_save_registration(data)
                entry={**{k:data.get(k) for k in
                    ("type","region","org","loan","apts","units",
                     "land","addr","anchor","arch","note","ts")},
                    "id":row_id}
                self._json(201,{"status":"ok","id":row_id,"entry":entry})
            except Exception as e:
                self._json(500,{"error":"Salvestamine ebaõnnestus: "+str(e)})
...
if __name__=="__main__":
    ...
    print(f"Spinnaker v7 on :{port}  db:{DB_PATH}",flush=True)
